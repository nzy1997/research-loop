...
function publicQuestion(event) {
  ...
  if (typeof id !== "string" || typeof prompt !== "string" || typeof answerType !== "string") return null;
  const result = { id, prompt, answerType };
  if (Array.isArray(choices) && choices.every((choice) => typeof choice === "string")) result.choices = choices;
  return result;
}
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (current.name === "prepare") {
        ...
        if (existing) {
          const currentJob = await jobStore.read(existing.jobId);
          if (!TERMINAL.has(currentJob.state)) return json(response, 202, await publicStatus({ rootDir, jobStore, job: currentJob }));
          activeByProblem.delete(current.problemId);
        }
        const created = await jobStore.create({ problemId: current.problemId, kind: "preparation" });
        const accepted = scheduler.enqueue(created);
        const selected = accepted?.jobId === created.jobId ? created : await jobStore.read(accepted.jobId);
        if (!TERMINAL.has(selected.state)) activeByProblem.set(current.problemId, selected);
      ...
      if (current.name === "input") {
        ...
        if (parent.state !== "needs_input") return error(response, 400, "job is not waiting for input.");
        const child = await jobStore.create({ problemId: parent.problemId, kind: "preparation", parentJobId: parent.jobId });
        const accepted = scheduler.resumeAfterInput({ ...child, answers });
        activeByProblem.set(parent.problemId, accepted);
        return json(response, 202, await publicStatus({ rootDir, jobStore, job: accepted }));
      }
      const job = current.name === "problem"
        ? activeByProblem.get(current.problemId) ?? (await jobStore.list()).filter((item) => item.problemId === current.problemId).at(-1)
