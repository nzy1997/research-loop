...
function activeQuestion(events) {
  return [...events].reverse().map(publicQuestion).find(Boolean) ?? null;
}

function validateQuestionAnswers(answers, question) {
  const entries = Object.entries(answers);
  if (entries.length !== 1 || entries[0][0] !== question.id) {
    throw new Error("answers must contain exactly the active question ID.");
  }
  if (question.answerType === "choice" && !question.choices.includes(entries[0][1])) {
    throw new Error("answer must be one of the active question's choices.");
  }
}
...
async function publicStatus({ rootDir, jobStore, job }) {
  ...
  if (job.state === "needs_input") {
    const question = activeQuestion(events);
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (current.name === "input") {
        ...
        const question = activeQuestion(await eventsFor(rootDir, parent.jobId));
        if (!question) throw new Error("job has no active persisted question.");
        validateQuestionAnswers(answers, question);
