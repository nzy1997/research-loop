...
export function createPreparationWorker({
  ...
  return async function prepare({ jobId, problemId, answers = {}, signal } = {}) {
    let job;
    try {
      ...
      const envelope = validatePreparationEnvelope(await codexAdapter({ stageDir: stage.stageDir, workspaceDir: stage.workspaceDir, problem, problemMarkdown, answers, signal }));
      assertNotAborted(signal);
      ...
      const report = await preflightRunner({ stageDir: stage.workspaceDir, manifest, privateDataRoot, signal });
      assertNotAborted(signal);
      ...
      const publication = await artifactStore.publishInfrastructureRevision({ rootDir, stageDir: stage.stageDir, problemId, expectedRevisionId: previewId, validateManifest: validateInfrastructureManifest, rebuildIndex, signal });
      ...
      if (job) {
        try { await jobStore.transition(jobId, signal?.aborted ? "interrupted" : "failed"); } catch { /* preserve the original preparation failure */ }
      }
      throw error;
    }
  };
