...
function publicInfrastructureId(value) {
  const match = typeof value === "string" ? INFRASTRUCTURE_ID.exec(value) : null;
...
export function buildPreparationPanelState({ problem, serviceState, localMode }) {
  ...
  if (jobState === "ready") {
    const revision = publicInfrastructureId(serviceState?.infrastructureId) ?? "Published infrastructure";
